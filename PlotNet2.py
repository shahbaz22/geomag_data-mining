...
class PlotNet: 
    ...
    def path_and_fhandel(self, comp, band):
        path = f'networks_data/{self.year}_nets/comp_{comp}/smr_min/'
        fhandel = f'{band}_{comp}_{self.n}_0.3_0.25_2.5_{self.year}smr_min.txt'
        return path, fhandel
...
for i in [0]:
    ...
    for comp in ['n','e','z']:
        lab = '_'.join(net_labs)
        pn.plot_dist_ts(indices_fname, net_labs, comp, y_space=10, save_plot=True, save_label=f'{lab}_smr_min')     
