...
# comp ='e'
# year='2015'
# fhandel = f'128_0.3_0.25_2.5_{year}.txt'
...
class DrawNetwork:
    ...
    def cartopy_ax(self, gridspec: plt.figure, date_time: str, G:nx.Graph, available_stations:list, proj: str, counts:dict, label:str, geographic:bool) ->None:
        ...
        if geographic == True:
            ...
            pos = {station: (map_proj.transform_point(v['AACGMLON'], v['AACGMLAT'], src_crs=source_proj))
               ...
               available_stations.to_dict('index').items()}
        # str split here to obtain pc_power
        # freqs = [counts[station]**1.8 for station in G.nodes()]
        # freqs = np.array(freqs)*0.01/1.5
        nx.draw_networkx_nodes(G = G, pos = pos, nodelist = G.nodes(), node_color='red', alpha = 0.8, node_size=10, ax = ax)
        nx.draw_networkx_edges(G = G, pos = pos, edge_color='y',
        alpha=1, arrows = self.directed, ax = ax, nodelist = G.nodes())
    ...
    def t_snapshot(self, target_time: str, label:str, index:int, save=False) -> None:
        ...
        # print(stations)
        file_path = 'networks_data/2015'
        nodes = [f.split('_')[1] for f in listdir(file_path) if isfile(join(file_path, f))]
        ...
        # counts = Counter(reshaped_edge_list)
        avail_stations = self.station_data[self.station_data.index.isin(nodes)]        
        print(avail_stations)
        G.add_nodes_from(nodes)      
        
        # ax4 = self.cartopy_ax(self.gs[1,1], date_time=target_time, G=G, available_stations=avail_stations, 
        #     counts=None, label = label, proj='globe', geographic=True)
        ...
        ax1 = self.cartopy_ax(self.gs[0,0], date_time=target_time, G=G, available_stations=avail_stations, 
            counts=None, label = label, proj='south_pole', geographic=True)
        # ax1 = self.plot_deg_dist(self.gs[0,0], target_time, xlims=[0,120], ylims=[0,35])
        ax2 = self.cartopy_ax(self.gs[0,1], date_time=target_time, G=G, available_stations=avail_stations, 
            counts=None, label = label, proj='north_pole', geographic=True) 
        ax3 = self.cartopy_ax(self.gs[1,0:], date_time=target_time, G=G, available_stations=avail_stations, 
            counts=None, label = label, proj='geomag', geographic=False)
